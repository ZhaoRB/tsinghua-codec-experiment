...
def parse_arg():
    ...
    parser.add_argument(
        "input_path",
        help="input path for a folder containing the multi-view images for a frame",
    )
    parser.add_argument("output", help="Name of the output video file")
    parser.add_argument(
        "-p",
        "--pattern",
        help="type of pattern used",
        type=int,
        nargs="+",
        choices=range(0, 26),
    )
    ...
    parser.add_argument(
        "-pad",
        help="Size of video to produce, input frame will be padded to reach this size, should be widthxheight",
    )
    parser.add_argument(
        "-crop",
        help="Size of video to produce, input frame will be cropped to reach this size, should be widthxheight",
    )
    args = parser.parse_args()
    return args


def gen_subjective_yuv():
    patterns = {
        0: [1, 7, 13, 19, 25],
    ...
    if args.pattern:
        if len(args.pattern) == 1:
            ...
            if p >= len(patterns):
                print(
                    "ERROR : please choose -p between 0 and {:d}".format(
                        len(patterns) - 1
                    )
                )
                p = 0
    ...
    if video_format == "yuv":
        cmd_ffmpeg = (
            "ffmpeg -framerate 30 -i " + temp + "/image_%03d.png -pix_fmt yuv420p "
        )
    else:
        cmd_ffmpeg = (
            "ffmpeg -f image2 -framerate 30 -i "
            + temp
            + "/image_%03d.png -vcodec libx264 -pix_fmt yuv420p "
        )

    if has_pad:
        cmd_ffmpeg += '-vf "pad={:d}:{:d}:(ow-iw)/2:(oh-ih)/2:color=gray" '.format(
            width, height
        )
